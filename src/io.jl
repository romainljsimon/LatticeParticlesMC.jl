...
function compute_box_str(box, ::EXYZ)
    if length(box) == 2
        return "$(box[1]) 0 0 0 $(box[2]) 0 0 0 0"
    elseif length(box) == 3
        return "$(box[1]) 0 0 0 $(box[2]) 0 0 0 $(box[3])"
    else
...
function write_header(io, system::Molecules, t, format::EXYZ, digits::Integer)
    ...
    println(io, "Lattice=\"$box_str\" Properties=molecule:I:1:species:S:1:pos:I:$(system.d) Time=$t")
    return nothing
end
